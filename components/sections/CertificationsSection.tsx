...
export async function CertificationsSection() {
    const { data: certifications } = await getCollection("certifications");
                                                    ...
                                                    <div className="mb-4">
                                                        <div className="flex flex-wrap justify-center gap-1.5">
                                                            {cert.skills.slice(0, 4).map((skill: any, idx: number) => {
                                                                return (
                                                                    <span
                                                                        key={`${cert.issuer}-${cert.name}-skill-${idx}`}
                                                                        className="inline-flex items-center gap-2 px-2 py-1 rounded-full bg-muted text-xs"
                                                                    >
                                                                        {skill}
                                                                    </span>
                                                                );
