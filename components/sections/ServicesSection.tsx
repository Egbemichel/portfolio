...
export async function ServicesSection() {
    const { data: services } = await getCollection("services");
                        ...
                        <div className="@container">
                            <div className="grid grid-cols-1 @3xl:grid-cols-2 gap-8">
                                {featured.map((service: any) => {
                                    const slug = service?.slug?.current ?? service?.slug ?? service?.id;
                                    return (
                                        <div
                                        ...
                                        >
                                            {service.icon && (
                                                <div className="relative w-12 h-12 @md/card:w-16 @md/card:h-16 mb-4 @md/card:mb-6">
                                                    <Image
                                                        src={buildImageUrl(service.icon, 64, 64)}
                                                ...
                                                <div className="mb-6">
                                                    ...
                                                    <ul className="space-y-2">
                                                        {service.features.map((feature: any, idx: number) => (
                                                             <li
                                                                 key={`${slug || service.title}-feature-${idx}`}
                                                        ...
                                                        {service.technologies.map((tech: any, idx: number) => {
                                                            const techData =
                                                             ...
                                                             return techData?.name ? (
                                                                 <span
                                                                     key={`${slug || service.title}-tech-${idx}`}
                                        ...
                                        >
                                            {service.icon && (
                                                <div className="relative w-10 h-10 @md/card:w-12 @md/card:h-12 mb-4">
                                                    <Image
                                                        src={buildImageUrl(service.icon, 48, 48)}
                                                        alt={service.title || "Service"}
                                                        fill
                                                        className="object-contain"
                                                    />
